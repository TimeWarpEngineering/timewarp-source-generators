...
{
  public const string DiagnosticId = "TW0002";
  ...
  private void AnalyzeTypeDeclaration(SyntaxNodeAnalysisContext context, TypeDeclarationSyntax typeDeclaration, SyntaxToken identifier)
  {
    ...
    SyntaxTriviaList trivia = typeDeclaration.GetLeadingTrivia();
    bool hasXmlDocs = trivia.Any(t => t.HasStructure && t.GetStructure() is DocumentationCommentTriviaSyntax);
    ...
    if (!hasXmlDocs)
    {
      ...
      {
        SyntaxTriviaList memberTrivia = member.GetLeadingTrivia();
